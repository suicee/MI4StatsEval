...
class Res_ANN_new(
    ...
    def __init__(self,
        ...
        for i in range(n_layers):
            if apply_dropout:
                fc_modules.append(
                    linear_dropout_layer(layer_dim, layer_dim, droput=dropout))
            else:
                fc_modules.append(nn.Linear(layer_dim, layer_dim))
...
class LSTM_Classifier_Res(torch.nn.Module):
    def __init__(self,
        ...
        self.fc_input_layer = nn.Linear(RNN_hidden_dim + param_dim,
                                        fc_layer_dim)

        self.fc_sequence = Res_ANN_new(n_layers=fc_layers,
                                       layer_dim=fc_layer_dim,
...
class ANN_Classifier_Res(torch.nn.Module):
    def __init__(self,
        ...
        self.fc_sequence = Res_ANN_new(n_layers=inter_fc_layers,
                                       layer_dim=fc_layer_dim,
                                       apply_dropout=apply_dropout,
                                       dropout=dropout)
...
class RNN_ANNclassifier(nn.Module):
    def __init__(self,
        ...
        self.rnn = nn.LSTM(feature_size,
                           ...
                           RNN_num_layers,
                           batch_first=True)
        self.ann_classifier = ANN_Classifier(RNN_hidden_dim, param_dim,
                                             fc_hidden_dims, apply_dropout,
                                             dropout)
...
class RNN_ResANNclassifier(nn.Module):
    def __init__(self,
        ...
        self.rnn = nn.LSTM(feature_size,
                           ...
                           RNN_num_layers,
                           batch_first=True)
        self.ann_classifier = ANN_Classifier_Res(RNN_hidden_dim, param_dim,
                                                 iter_fc_layers,
                                                 fc_hidden_dims, apply_dropout,
                                                 dropout)
